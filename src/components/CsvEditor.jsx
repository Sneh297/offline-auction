...
// Only add Drive-specific attributes if the URL is from Google
const imgProps = (url) => {
...
export default function CsvEditor() {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.ctrlKey && e.key === "s") { e.preventDefault(); SaveToLocalStorageCSV(); }
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        const parsedData = results.data.map((row) => ({
          ...row,
          ...(row.photourl ? { photourl: convertDriveUrl(row.photourl) } : {}),
        }));
        setData(parsedData);
        setColumns(Object.keys(parsedData[0]));
        localStorage.setItem("playerDetails", Papa.unparse(parsedData));
        alert("CSV uploaded and saved to local storage!");
      },
    });
  };
  ...
  const handleChange = (rowIndex, column, value) => {
    const updated = [...data];
    updated[rowIndex][column] = column === "photourl" ? convertDriveUrl(value) : value;
    setData(updated);
  };

  const SaveToLocalStorageCSV = () => {
    const emptyCells = getEmptyCells();
    if (emptyCells.length > 0) {
      alert(`❌ Cannot save — ${emptyCells.length} empty cell(s):\n\n${emptyCells.map(({ rowIndex, col }) => `Row ${rowIndex + 1} → "${col}"`).join("\n")}`);
      return;
    }
    localStorage.setItem("playerDetails", Papa.unparse(data));
    alert("✅ CSV data saved to local storage!");
  };
